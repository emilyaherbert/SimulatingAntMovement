...
class SimilarSizeKMeans(private val numClusters: Int,
  ...
  def runClustering(data: Dataset[Row],
  ...
  def isEvenlyWeighted(predictions: Dataset[Row]): (Boolean, Array[(Int,Int)], Double, Double) = {
  ...
  def splitClusters(predictions: Dataset[Row],
